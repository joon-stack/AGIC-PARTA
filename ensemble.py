    ...
    for train_idx, val_idx in kf.split(output_labels):
        ...
        if isTrain:
            ...
            for i in range(10):
                ...
                state_dict = torch.load('models/param{}.data'.format(str(i+1)))
            ...
            for epoch in range(args.epochs):
                ...
                with torch.no_grad():
                    ...
                    nmse_tmp = evaluate_ensemble(eval_args)

                    if nmse_tmp < nmse_save:
                        earlyStoppingCount = 0
                        best_epoch = epoch
                        nmse_save = copy(nmse_tmp)
                        torch.save(model.state_dict(), './models/ensemble{}.data'.format(fold))
